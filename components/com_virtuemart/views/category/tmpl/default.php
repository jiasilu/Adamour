...
if (VmConfig::get ('showCategory', 1) and empty($this->keyword)) {
	if ($this->category->haschildren) {
		...
		foreach ($this->products as $product) { 
			?>
		<div class="category-view-top"><?php
    	echo $product->images[0]->displayMediaFull('class="medium-image" title="' . $product->product_name . '" ', TRUE, 'class="modal"');
 		?>
		<h2><?php echo JHTML::link ($product->link, $product->product_name); ?></h2>
      	<?php
		echo $this->currency->createPriceDiv ('priceWithoutTax', 'COM_VIRTUEMART_PRODUCT_SALESPRICE', $product->prices);
		?>
		</div>
		<?php 
		}
